...
public:
	...
	public:
		enum t_te
		{
			te_bol, te_header_left_marker, te_header_name, te_header_right_marker,
			te_key_name, te_key_marker, te_key_value, te_error, te_comment, te_eol, te_unknown, 
			te_first = te_header_left_marker, te_count = te_unknown
		};
		...
		int c_index(int te) const
		{
			return m_index[te];
		}

		int cc_index(int te) const
		{
			return m_index[te + 1] - m_index[te];
		}

		string t_index(int te) const
		{
...
public:
	...
	const SLineInfo GetLine(int nLine) const
	{
	...
	LPCTSTR GetLineChars(int nLine);
	...
	virtual void UpdateViews(CCrystalTextView *pSource, CUpdateContext *pContext,
