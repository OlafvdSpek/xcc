...
int Cucr_container::open(const Cvirtual_binary& d, const Cucr_format& format)
{
	if (!m_objects.empty())
		close();
	if (!d)
	...
	int c = 0;
	{
		...
		for (Cucr_format::t_blocks::const_iterator i = format.blocks().begin(); i != format.blocks().end(); i++)
		{
			if (i->offset)
			...
			while (k--)
			{
				...
				if (i->update_object)
					temp_object = m_objects.find(Cucr_container::offset(object_i))->second;
				...
				if (i->add_object)
				{
					...
					m_objects[offset] = Cucr_object(d, name, offset, size);
					m_offsets.push_back(offset);
				}
				else if (i->update_object)
					m_objects[Cucr_container::offset(object_i)] = temp_object;
				object_i++;
			}
		}
