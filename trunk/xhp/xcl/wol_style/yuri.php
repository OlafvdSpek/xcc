...
$query7 = db_query("SELECT mtime, name scen, ws_gid, gid FROM xcl_games inner join xcl_maps using (mid) WHERE gid='$data3[gid]' AND trny='1'");
...
$id = addslashes($id);
$query = db_query("SELECT g.*, m.name scen FROM xcl_games g inner join xcl_maps m using (mid) WHERE gid='$id'");
while ($data = mysql_fetch_array($query)) {
$gametype = $data[trny];
?>
<table class="maintitle" width="100%" border="0" cellspacing="1" cellpadding="2">
