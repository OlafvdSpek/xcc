...
Cedit_unit_properties_dlg::Cedit_unit_properties_dlg(Cxcc_level& level)
	...
	m_level(level)
{
	//{{AFX_DATA_INIT(Cedit_unit_properties_dlg)
	m_edit_angle = 0;
	m_edit_cell = _T("");
	m_edit_health = 0;
	m_edit_trigger = _T("");
...
void Cedit_unit_properties_dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//{{AFX_DATA_MAP(Cedit_unit_properties_dlg)
...
BOOL Cedit_unit_properties_dlg::OnInitDialog() 
{
	...
	m_edit_type = xcc_units::unit_data[m_d.t].long_name;
...
void Cedit_unit_properties_dlg::OnOK() 
{
	if (!UpdateData(true))
		return;
...
void Cedit_unit_properties_dlg::OnSelect() 
{
