...
void CCrystalTextBuffer::SLineInfo::parse()
{
	const string& line = text();
	for (int te = 0; te < te_count; te++)
		m_index[te] = string::npos;
	m_index[te_bol] = 0;
	m_index[te_eol] = line.length();
	int i = 0;
	int first_non_ws;
	int last_non_ws;
	while (i < line.length())
	{
		...
		case ' ':
			break;
		case ';':
			m_index[te_comment] = i;
			return;
		case '=':
			m_index[te_error] = i;
			return;
		case '[':
			m_index[te_header_left_marker] = i;
			last_non_ws = i++;
			first_non_ws = i;
			while (i < line.length())
			{
			...
			if (line[last_non_ws] != ']')
			{
				m_index[te_error] = last_non_ws;
				return;
			}
			m_index[te_header_name] = first_non_ws;
			m_index[te_header_right_marker] = last_non_ws;
			return;
		default:
			...
			while (i < line.length())
			{
				...
				case '=':
					{
						m_index[te_key_name] = first_non_ws;
						m_index[te_key_marker] = i;
						...
						while (i < line.length())
						{
							...
							default:
								...
								m_index[te_key_value] = first_non_ws;
								return;
							}
							i++;
...
void CCrystalTextBuffer::InsertLine(LPCTSTR pszLine, int nLength /*= -1*/, int nPosition /*= -1*/)
{
	...
	SLineInfo li = string(pszLine, nLength);
...
void CCrystalTextBuffer::AppendLine(int nLineIndex, LPCTSTR pszChars, int nLength /*= -1*/)
{
	...
	li = li.line() + string(pszChars, nLength);
}
...
BOOL CCrystalTextBuffer::SaveToFile(LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/, BOOL bClearModifiedFlag /*= TRUE*/)
{
	...
	__try
	{
		...
		for (int nLine = 0; nLine < nLineCount; nLine ++)
		{
			...
			if (nLength > 0)
			{
				if (! ::WriteFile(hTempFile, T2A(const_cast<char*>(m_aLines[nLine].chars())), nLength, &dwWrittenBytes, NULL))
					__leave;
...
LPCTSTR CCrystalTextBuffer::GetLineChars(int nLine)
{
	ASSERT(m_bInit);	//	Text buffer not yet initialized.
						//	You must call InitNew() or LoadFromFile() first!
	return m_aLines[nLine].chars();
}
...
void CCrystalTextBuffer::GetText(int nStartLine, int nStartChar, int nEndLine, int nEndChar, CString &text, LPCTSTR pszCRLF /*= NULL*/)
{
	...
	{
		int nCount = GetLineLength(nStartLine) - nStartChar;
		if (nCount > 0)
		{
			memcpy(pszBuf, GetLineChars(nStartLine) + nStartChar, sizeof(TCHAR) * nCount);
			pszBuf += nCount;
		}
		...
		if (nEndChar > 0)
		{
			memcpy(pszBuf, GetLineChars(nEndLine), sizeof(TCHAR) * nEndChar);
			pszBuf += nEndChar;
		}
	}
	else
...
BOOL CCrystalTextBuffer::InternalDeleteText(CCrystalTextView *pSource, int nStartLine, int nStartChar, int nEndLine, int nEndChar)
{
	...
	if (nStartLine == nEndLine)
	{
		...
		string v = li.line();
		v.erase(nStartChar, nEndChar - nStartChar);
		li = v;
	...
	{
		int nRestCount = GetLineLength(nEndLine) - nEndChar;
		LPTSTR pszRestChars = NULL;
		if (nRestCount > 0)
		{
			pszRestChars = new TCHAR[nRestCount];
			memcpy(pszRestChars, GetLineChars(nEndLine) + nEndChar, nRestCount * sizeof(TCHAR));
		}
		...
		*/
		...
		string v = m_aLines[nStartLine].line();
		v.erase(nStartChar);
		m_aLines[nStartLine] = v;
		if (nRestCount > 0)
		{
...
BOOL CCrystalTextBuffer::InternalInsertText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar)
{
	...
	int nRestCount = GetLineLength(nLine) - nPos;
	LPTSTR pszRestChars = NULL;
	if (nRestCount > 0)
	{
		pszRestChars = new TCHAR[nRestCount];
		memcpy(pszRestChars, GetLineChars(nLine) + nPos, nRestCount * sizeof(TCHAR));
		string v = m_aLines[nLine].line();
		v.erase(nPos);
		m_aLines[nLine] = v;
	}
	...
	for (;;)
	{
		...
		if (pszText[nTextPos] == 0)
		{
			...
			nEndChar = GetLineLength(nCurrentLine);
...
int CCrystalTextBuffer::FindNextBookmarkLine(int nCurrentLine)
{
	...
	{ 
		...
		{
			if ((GetLineFlags(nCurrentLine) & LF_BOOKMARKS) != 0)
				return nCurrentLine;
			// Keep going
			nCurrentLine++;
		}
